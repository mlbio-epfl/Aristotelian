...
class TestLcsKnn:
    def test_matches_original(self, normalized_features):
        feats_A, feats_B = normalized_features
        topk = 10
        ours = lcs_knn(feats_A, feats_B, topk=topk)
        # Original returns tensor, convert for comparison
        original = OriginalMetrics.lcs_knn(feats_A, feats_B, topk)
        if isinstance(original, torch.Tensor):
            original = float(original.item())
        assert np.isclose(ours, original, rtol=1e-5)


class TestEditDistanceKnn:
    def test_matches_original(self, normalized_features):
        feats_A, feats_B = normalized_features
        topk = 10
        ours = edit_distance_knn(feats_A, feats_B, topk=topk)
        # Original returns tensor, convert for comparison
        original = OriginalMetrics.edit_distance_knn(feats_A, feats_B, topk)
        if isinstance(original, torch.Tensor):
            original = float(original.item())
        assert np.isclose(ours, original, rtol=1e-5)
...
class TestSVCCA:
    def test_matches_original(self, normalized_features):
        feats_A, feats_B = normalized_features
        # Set seeds for reproducibility (due to random noise addition)
        torch.manual_seed(0)
        np.random.seed(0)
