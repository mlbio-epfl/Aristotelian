...
class TestCKAEstimatorsBasic:
    ...
    def test_identical_matrices_high_score(self):
        ...
        assert cka_debiased(X, X) > 0.95  # May be slightly lower due to bias correction
        # Note: cka_depcols with indep_cols=False on identical matrices is a degenerate case
        # and may give low values due to the column-dependence correction
    ...
    def test_random_independent_matrices(self):
        ...
        # Both should be relatively low for independent random matrices
        # Biased may be higher, debiased should be closer to 0
        assert biased < 0.6
        assert abs(debiased) < 0.5
        # Biased should generally be >= debiased
    ...
    def test_random_matrices_debiased_near_zero(self):
        ...
        # Debiased should be closer to 0 than biased
...
class TestCKAEstimatorsHighDNRegime:
    ...
    def test_high_dn_biased_inflates(self):
        ...
        # Biased should be much higher than debiased in this regime
        assert biased > 0.3  # Inflated value
        assert abs(debiased) < 0.3  # Should be near 0
    ...
    def test_increasing_dn_ratio(self):
        ...
        # Biased should generally increase
        assert biased_values[-1] > biased_values[0]

        # Debiased should stay relatively stable near 0
...
class TestCKAEstimatorsConsistency:
    ...
    def test_moment_estimators_internally_consistent(self):
        ...
        # With indep_cols=True, depcols should equal song
    ...
    def test_gram_based_biased_vs_debiased(self):
        ...
        torch.manual_seed(42)
        # Use high d/n to see the difference
        ...
        # Biased should be higher (inflated) in high d/n regime
