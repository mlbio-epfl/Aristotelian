...
def test_edit_distance_knn_matches_legacy(random_data):
    """Test that unified edit_distance_knn matches legacy implementation."""
    from aristotelian import edit_distance_knn

    X, Y = random_data
    k = 10

    # Legacy
    legacy_result = edit_distance_knn(X, Y, topk=k)

    # Unified
    config = MetricConfig(topk=k)
    unified_result = MetricRegistry.compute_raw("edit_distance_knn", X, Y, config)
...
def test_svcca_matches_legacy(random_data):
    """Test that unified svcca matches legacy prh_metrics.svcca."""
    ...
    # Unified sklearn version (matches PRH implementation)
    config = MetricConfig(cca_dim=10)
    unified_result = MetricRegistry.compute_raw("svcca_sklearn", X, Y, config)

    # SVCCA involves randomness in sklearn, so allow some tolerance
    assert abs(legacy_result - unified_result) < 0.1
