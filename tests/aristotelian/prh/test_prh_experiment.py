...
def test_compute_outlier_ablation_returns_summary():
    torch.manual_seed(1)
    feats_x = [torch.randn(10, 2, 4), torch.randn(10, 2, 4)]
    feats_y = [torch.randn(10, 2, 4), torch.randn(10, 2, 4)]
    out = compute_outlier_ablation(
        feats_x,
        feats_y,
        q_values=[0.8, 0.95, 1.0],
        topk=3,
        num_permutations=5,
        alpha=0.2,
        seed=0,
    ...
    assert out["q_values"] == [0.8, 0.95, 1.0]
    assert len(out["mean_gated"]) == 3
    assert len(out["mean_raw"]) == 3
