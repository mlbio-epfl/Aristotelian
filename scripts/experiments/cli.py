...
def _run_section_task(
    ...
    with log_timing(f"Section: {section}"):
        ...
        # V2V arguments
        if "v2v_metrics" in sig.parameters:
            kwargs["v2v_metrics"] = prh_metrics  # reuse PRH metrics
        if "v2v_modelset" in sig.parameters:
            kwargs["v2v_modelset"] = v2v_modelset
        if "v2v_q_outlier" in sig.parameters:
            kwargs["v2v_q_outlier"] = prh_q_outlier  # reuse PRH outlier quantile
        if "data_dir" in sig.parameters:
            kwargs["data_dir"] = v2v_data_dir
        # V2T arguments
...
def main() -> None:
    ...
    # V2V (vision-to-vision) experiment arguments
    parser.add_argument(
        "--v2v-modelset",
        default="default",
        choices=[
        ...
        help="Vision model set for V2V experiment (default: default).",
    )
    parser.add_argument(
        "--v2v-data-dir",
        default=None,
        help="Local Places365 data directory (optional, uses HuggingFace by default).",
    )
        ...
        for idx, section in enumerate(sections, 1):
            ...
            with log_timing(f"Section: {section}"):
                ...
                if "v2v_metrics" in sig.parameters:
                    kwargs["v2v_metrics"] = prh_metrics  # reuse PRH metrics
                if "v2v_modelset" in sig.parameters:
                    kwargs["v2v_modelset"] = args.v2v_modelset
                if "v2v_q_outlier" in sig.parameters:
                    kwargs["v2v_q_outlier"] = prh_q_outlier  # reuse PRH outlier
                if "data_dir" in sig.parameters:
                    kwargs["data_dir"] = args.v2v_data_dir
                # V2T arguments
                if "v2t_metrics" in sig.parameters:
                    kwargs["v2t_metrics"] = prh_metrics  # reuse PRH metrics
                if "v2t_video_modelset" in sig.parameters:
