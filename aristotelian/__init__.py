...
# CKA estimator variants for comparison studies
...
def lcs_knn(feats_A: torch.Tensor, feats_B: torch.Tensor, *, topk: int) -> float:
    """Longest common subsequence of kNN neighbor lists."""
    return MetricRegistry.compute_raw(
        "lcs_knn", feats_A, feats_B, MetricConfig(topk=topk)
    )


def edit_distance_knn(
    feats_A: torch.Tensor, feats_B: torch.Tensor, *, topk: int
) -> float:
    """Edit distance normalized kNN similarity."""
    return MetricRegistry.compute_raw(
        "edit_distance_knn", feats_A, feats_B, MetricConfig(topk=topk)
    )
...
def svcca(feats_A: torch.Tensor, feats_B: torch.Tensor, *, cca_dim: int = 10) -> float:
    """Singular Vector CCA."""
    return MetricRegistry.compute_raw(
        "svcca_sklearn", feats_A, feats_B, MetricConfig(cca_dim=cca_dim)
    )
...
from .metrics.ablation import run_preprocess_ablation
