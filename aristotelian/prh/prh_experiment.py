...
def run_prh_outlier_ablation(
    *,
    ...
    num_permutations: int = 200,
    alpha: float = 0.05,
    q_values: Sequence[float] = (0.8, 0.9, 0.95, 0.98, 1.0),
    force: bool = False,
    force_features: bool = False,
    ...
    feats_x = []
    feats_y = []
    for model_name in tqdm(models_x, desc=f"Loading {modality_x} features"):
        out_path = prh_feature_filename(
            os.path.join(output_dir, "features"),
            dataset,
            subset,
    ...
    for model_name in tqdm(models_y, desc=f"Loading {modality_y} features"):
        out_path = prh_feature_filename(
            os.path.join(output_dir, "features"),
            dataset,
            subset,
    ...
    summary = compute_outlier_ablation(
        feats_x,
        feats_y,
        q_values=q_values,
        metric=metric,
